...
当前仓库已完成 `Phase 1: Shared Core`，并已落地 `Phase 2 / Task 2` 的 `stocks` 历史 batch endpoint，核心文件和目录如下：
...
- `src/stocks/`：第一个开始真实实现的资源域，当前已包含 `bars` / `quotes` / `trades` 历史 batch request、typed response/model、query 序列化和 client fetcher；single/latest/snapshot/metadata 仍在后续 task
...
- `stocks` 的 single historical、latest、snapshot、metadata endpoint 实现
- `options`、`news`、`corporate_actions` 的真实 HTTP endpoint 实现
- 除 `crypto.latest_quotes` 与 `stocks` 历史 batch 之外的完整 Market Data 请求/响应字段模型
- 资源级 benchmark 基线
...
- 当前真正落地的真实能力已覆盖共享层、`crypto.latest_quotes`，以及 `stocks` 历史 batch 的 `bars` / `quotes` / `trades`。
- 当前 `stocks` 的 single historical、latest、snapshot、metadata 和其余资源域方法仍以占位壳为主，不能当成真实 HTTP 逻辑已完成。
