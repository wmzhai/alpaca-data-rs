...
- `stocks` 与 `options` 现已成为前两个完整资源模板；`crypto` 当前已完成 historical batch + convenience 的公开边界，并明确 `loc` 只作为 path 路由而不进入 query；后续 `news`、`corporate_actions` 默认沿用这套 mirror + convenience + live/mock/benchmark 的公开边界
