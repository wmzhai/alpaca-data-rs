...
- 这是一个新项目，当前已经完成 `Phase 1: Shared Core`、`Phase 2: Stocks`、`Phase 3: Options` 与 `Phase 4: Crypto`，并已形成 `Phase 5: News + Corporate Actions` 的收尾版本候选，当前工作版本是 `v0.5.0`。
...
- 当前 `Phase 5: News + Corporate Actions` 的收尾版本候选已经准备好：`news` 与 `corporate_actions` 现在都已具备真实 HTTP 行为、分页便利层、fault injection baseline 与本地 benchmark baseline；在 merge 获批前，下一步默认转入 `Phase 6: Release Hardening`。
