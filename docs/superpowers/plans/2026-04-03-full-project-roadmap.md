...
- **Phase 4: Crypto**，进行中（historical done in `v0.3.1`）
...
- `src/transport/endpoint.rs` 已提供 crypto historical `bars` / `quotes` / `trades` 与 `latest_quotes` 的官方路径路由
...
- `tests/live_crypto_latest_quotes_smoke.rs` 与 `tests/live_crypto_historical.rs` 已验证 `crypto.latest_quotes` 与 crypto historical happy path
