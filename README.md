...
- 当前已完成 `Phase 1: Shared Core`，并已进入 `Phase 2: Stocks`；当前已落地到 `v0.1.2`
- 已落地共享 `ClientBuilder` 运行时配置、认证配对校验与 header 注入、query 构造、endpoint 路由、async HTTP transport、错误映射和分页 helper
- 当前真实打通的 endpoint 包括 `crypto.latest_quotes`，以及 `stocks.bars`、`stocks.quotes`、`stocks.trades`
- 真实 happy-path 测试已覆盖 `crypto.latest_quotes` 和 `stocks` 历史 batch endpoint
